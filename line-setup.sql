    ...
    loop
        ...
        if r.checkin = 'Y' then
            perform public.line_try_send(r.uid, r.code, 'checkin',
                '✅ Check-in สำเร็จ' || E'\n' ||
                'ผลงาน: ' || r.code || E'\n' ||
                'ระบบได้บันทึกการ Check-in ของท่านเรียบร้อยแล้ว กรุณามารอ ณ จุดนำเสนอ ก่อนเวลานำเสนออย่างน้อย 15 นาที',
                v_token);
        end if;
        ...
        if v_end is not null and r.checkin = 'Y'
           and coalesce(r.reward,'') <> 'Y' and v_now > v_end then
            perform public.line_try_send(r.uid, r.code, 'ended',
                '📜 รับใบประกาศนียบัตร' || E'\n' ||
                'ผลงาน: ' || r.code || E'\n' ||
                'กรุณาติดต่อจุด Check-in เพื่อรับใบประกาศนียบัตร พร้อมแสดงคูปองและ QR Code ลงทะเบียนแก่เจ้าหน้าที่',
                v_token);
        end if;
        ...
        if r.reward = 'Y' then
            perform public.line_try_send(r.uid, r.code, 'reward',
                '🙏 ขอบคุณ' || E'\n' ||
                'ขอขอบคุณที่ร่วมส่งผลงานและเป็นส่วนหนึ่งของการประชุมวิชาการวิทยาศาสตร์การแพทย์ ครั้งที่ 34' || E'\n' ||
